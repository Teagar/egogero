...
8. Deploy one instance, require readiness success, verify liveness reveals no dependency detail, then continue the rollout. Exercise login, callback, MFA-role enforcement, recovery webhook, and gatehouse HTTPS rejection. Before a human-auth canary, `AUTH_ROLLOUT_MODE=canary`, the injected instance/stage IDs, protected durable snapshot path, `AUTH_ALERT_ADAPTER=https_webhook`, and external smoke acknowledgement ID are mandatory. Run `npm run auth:alerts:smoke -- --url "$AUTH_ALERT_WEBHOOK_URL"`; every bounded type must reach and be acknowledged by its real operator destination. The runtime default `stdout` is suitable only for explicit non-canary/provider-neutral plumbing.

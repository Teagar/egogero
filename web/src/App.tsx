...
function Notice({ children, kind = 'error' }: { children: ReactNode; kind?: 'error' | 'success' | 'warning' | 'info' }) { return <div className={`notice ${kind}`} role="alert">{children}</div>; }
...
function LoadingBlock({ label = 'Carregando dados autorizados...' }: { label?: string }) { return <div className="empty loading-block" aria-busy="true" role="status"><span className="loading-mark" aria-hidden="true" />{label}</div>; }
...
function Page({ title, eyebrow, description, action, children }: { title: string; eyebrow: string; description?: string; action?: ReactNode; children: ReactNode }) { const heading = useRef<HTMLHeadingElement>(null); useEffect(() => { heading.current?.focus({ preventScroll: true }); }, [title, eyebrow]); return <><header className="page-header blueprint"><div><p className="eyebrow">{eyebrow}</p><h1 className="page-title" ref={heading} tabIndex={-1}>{title}</h1>{description && <p className="page-description">{description}</p>}</div>{action}</header><div className="page-content">{children}</div></>; }
...
function AuthStage({ children }: { children: ReactNode }) {
  return <main className="auth-stage blueprint" id="content"><a className="skip" href="#auth-card">Ir para o conteúdo</a><aside className="auth-rail" aria-label="Plataforma de acesso"><Brand /><div><p className="eyebrow">Controle de acesso humano</p><p className="auth-rail-title">Contexto explícito.<br />Entrada controlada.</p><p>Identidade centralizada para a operação de condomínios, moradores e portarias.</p></div><p className="auth-rail-meta">OIDC / sessão opaca / RBAC</p></aside><section className="auth-main"><div className="auth-card" id="auth-card"><div className="auth-card-brand"><Brand /></div>{children}</div></section></main>;
}
...
function LoginPage({ kind = 'login' }: { kind?: 'login' | 'error' | 'recovery' }) {
  ...
  return <AuthStage><span className={`status ${kind === 'error' ? 'danger' : ''}`}>{kind === 'recovery' ? 'Recuperação' : kind === 'error' ? 'Falha temporária' : 'Acesso OIDC'}</span><div className="auth-copy"><p className="eyebrow">Identidade corporativa</p><h1>{title}</h1><p className="muted">{kind === 'recovery' ? 'Recuperação e credenciais são administradas pelo provedor de identidade.' : 'Use sua identidade corporativa. Nenhuma senha é mantida por esta plataforma.'}</p></div>{kind === 'error' && <Notice>A autenticação está indisponível no momento. Tente novamente sem alterar seus dados de acesso.</Notice>}<a className="button primary" href={kind === 'recovery' ? '/auth/recovery' : '/auth/login'}>{kind === 'recovery' ? 'Continuar recuperação' : kind === 'error' ? 'Tentar novamente' : 'Continuar com identidade corporativa'}</a>{kind !== 'recovery' && <div className="auth-help"><span>Precisa de ajuda</span><a href="/recovery">Não consigo acessar minha conta</a></div>}</AuthStage>;
}
...
function InvitationPage() {
  ...
  return <AuthStage><span className={`status ${state === 'unavailable' ? 'danger' : 'warning'}`}>Convite administrativo</span><div className="auth-copy"><p className="eyebrow">Provisionamento / 24 horas</p><h1>Ative seu vínculo.</h1><p className="muted">Este link provisiona acesso à plataforma. Ele não é o código de seis dígitos usado por visitantes.</p></div>{state === 'unavailable' ? <><Notice>Este convite não está disponível. Solicite um novo convite à administração.</Notice><a className="button secondary" href="/login">Ir para o acesso normal</a></> : <button className="primary" aria-busy={state === 'working'} disabled={state === 'working'} onClick={submit}>{state === 'working' ? 'Conectando ao provedor...' : 'Continuar com identidade corporativa'}</button>}</AuthStage>;
}

function SecuritySurface({ state, retry, retryAfter }: { state: AuthStateName; retry: () => void; retryAfter: number | null }) {
  const text: Record<AuthStateName, { title: string; message: string; label: string; kind: 'info' | 'warning' | 'danger'; action: string }> = {
    unauthenticated: { title: 'Entrar na plataforma', message: 'Continue com a identidade corporativa para acessar um vínculo autorizado.', label: 'Acesso OIDC', kind: 'info', action: 'Continuar com identidade corporativa' },
    'session-expired': { title: 'Sessão encerrada', message: 'Por segurança, entre novamente para continuar no seu contexto.', label: 'Sessão expirada', kind: 'warning', action: 'Entrar novamente' },
    'bootstrap-unavailable': { title: 'Serviço indisponível', message: 'Não foi possível verificar sua sessão agora. Nenhum contexto foi carregado.', label: 'Falha temporária', kind: 'danger', action: 'Tentar novamente' },
    'rate-limited': { title: 'Acesso temporariamente limitado', message: retryAfter ? `Aguarde cerca de ${retryAfter} segundos antes de tentar novamente.` : 'Aguarde antes de tentar novamente.', label: 'Proteção ativa', kind: 'warning', action: 'Tentar novamente' },
    'reauth-required': { title: 'Confirmação necessária', message: 'Esta ação exige uma validação recente da sua identidade.', label: 'Reautenticação', kind: 'info', action: 'Confirmar identidade' },
    'mfa-insufficient': { title: 'Verificação adicional necessária', message: 'Seu método atual não atende à política deste contexto.', label: 'MFA necessário', kind: 'warning', action: 'Continuar verificação' },
    'membership-unavailable': { title: 'Contexto indisponível', message: 'Seu acesso ou contexto não está disponível no momento. A resposta não diferencia vínculo inexistente ou sem permissão.', label: 'Acesso indisponível', kind: 'danger', action: 'Verificar novamente' }
  };
  const current = text[state];
  return <AuthStage><span className={`status ${current.kind}`}>{current.label}</span><div className="auth-copy"><p className="eyebrow">Estado de segurança</p><h1>{current.title}</h1><p className="muted">{current.message}</p></div>{state === 'unauthenticated' || state === 'session-expired' ? <a className="button primary" href="/auth/login">{current.action}</a> : <button className="primary" onClick={retry}>{current.action}</button>}<p className="auth-assurance">A plataforma não solicita senha local nem revela detalhes internos de autorização.</p></AuthStage>;
}
...
function ProviderPage() {
  ...
  return <Page title="Operações da rede" eyebrow="R01 / Papel: provedor" description="Cadastre e acompanhe os condomínios autorizados sem entrar implicitamente em um tenant." action={resource.state.status === 'success' ? <span className="status">{resource.state.data.length} cadastrados</span> : undefined}><div className="split"><section className="panel signal"><p className="eyebrow">Provisionamento</p><h2>Novo condomínio</h2><form onSubmit={create} className="form-grid" aria-busy={busy}><Field name="nome" label="Nome" disabled={busy} /><Field name="responsavel" label="Responsável" disabled={busy} /><Field name="tipo" label="Tipo" disabled={busy} /><Field name="timezone" label="Fuso horário" defaultValue="America/Sao_Paulo" disabled={busy} /><button className="primary" disabled={busy}>{busy ? 'Cadastrando...' : 'Cadastrar condomínio'}</button></form>{message && <Notice>{message}</Notice>}</section><section><p className="eyebrow">Inventário autorizado</p><h2>Rede ativa</h2>{resource.state.status === 'loading' ? <LoadingBlock /> : resource.state.status === 'error' ? <ErrorBlock message={resource.state.message} retry={resource.retry} /> : resource.state.data.length ? <div className="card-list">{resource.state.data.map((item) => <article className="data-card" key={item.id}><p className="eyebrow">{item.tipo}</p><h3>{item.nome}</h3><p>{item.responsavel}</p><span className="mono muted">{item.timezone}</span></article>)}</div> : <Empty>Nenhum condomínio disponível.</Empty>}</section></div></Page>;
}
...
function ResidentPage({ tenantId, residentId }: { tenantId: string; residentId: string }) {
  ...
  if (resource.state.status === 'loading') return <Page title="Meus convidados" eyebrow="R03 / Papel: morador" description="Autorize visitas com período definido e compartilhe somente o código gerado."><LoadingBlock /></Page>;
  if (resource.state.status === 'error') return <Page title="Meus convidados" eyebrow="R03 / Papel: morador" description="Autorize visitas com período definido e compartilhe somente o código gerado."><ErrorBlock message={resource.state.message} retry={resource.retry} /></Page>;
...
function Shell({ session, refresh }: { session: Session; refresh: () => void }) {
  ...
  const content = page === 'condominiums' ? <ProviderPage /> : page === 'people' ? <ManagerPage tenantId={active.tenantId!} /> : page === 'guests' ? <ResidentPage tenantId={active.tenantId!} residentId={active.residentId!} /> : page === 'validation' ? <GatehousePage /> : <Page title="Trocar contexto" eyebrow="Conta / vínculos autorizados" description="Escolha explicitamente o condomínio e o papel usados nesta sessão."><div className="context-grid">{session.memberships.map((membership) => { const selected = membership.id === active.id; return <button key={membership.id} className={`context-card ${selected ? 'active' : ''}`} aria-pressed={selected} aria-label={`${selected ? 'Contexto ativo' : 'Entrar no contexto'}: ${membership.tenantLabel ?? 'Rede global'}, ${roleNames[membership.role]}`} disabled={switching} onClick={() => switchMembership(membership.id)}><span className="eyebrow">{roleNames[membership.role]}</span><strong>{membership.tenantLabel ?? 'Rede global'}</strong><small>{membership.residentLabel ?? 'Contexto administrativo'}</small>{selected && <span className="status success">Ativo nesta sessão</span>}</button>; })}</div></Page>;
  return <div className="shell"><a className="skip" href="#content">Ir para o conteúdo</a><header className="topbar"><Brand /><div className="topbar-context"><span className="context-label">Contexto operacional</span><strong>{context}</strong><span className="topbar-role">{roleNames[active.role]}</span></div><details className="account-details"><summary aria-label={`Conta de ${session.account.displayName}`}><span>Conta</span><strong>{session.account.displayName}</strong></summary><nav aria-label="Conta"><span className="context-label">Identidade ativa</span><strong>{session.account.displayName}</strong><small>{context} · {roleNames[active.role]}</small><a href="/logout">Opções de saída</a></nav></details></header><aside className="sidebar"><nav aria-label="Navegação principal">{nav.map(([key, label], index) => <button key={key} data-destination={key} aria-current={page === key ? 'page' : undefined} onClick={() => setPage(key as never)}><span className="nav-index">{String(index + 1).padStart(2, '0')}</span>{label}</button>)}</nav><div className="side-context"><span className="context-label">Contexto ativo</span><strong>{context}</strong><small>{roleNames[active.role]} · {session.account.displayName}</small><span className="status success">Sessão ativa</span></div></aside><main id="content" className="shell-main" aria-busy={switching}>{error && <Notice>{error}</Notice>}{switching ? <Page title="Trocando contexto" eyebrow="Sessão / rotação segura" description="Os dados do vínculo anterior são removidos antes da próxima superfície ser carregada."><p role="status" aria-live="polite">Trocando contexto e removendo os dados anteriores.</p><LoadingBlock label="Limpando o contexto anterior..." /></Page> : <div key={active.id}>{content}</div>}</main><nav className="mobile-nav" aria-label="Navegação móvel">{nav.map(([key, label], index) => <button key={key} data-destination={key} aria-current={page === key ? 'page' : undefined} disabled={switching} onClick={() => setPage(key as never)}><span aria-hidden="true">{String(index + 1).padStart(2, '0')}</span>{label}</button>)}</nav></div>;
}
...
export function App() {
  ...
  if (state.name === 'checking') return <AuthStage><span className="status">Verificação de sessão</span><div className="auth-copy" aria-busy="true" role="status"><p className="eyebrow">Inicialização segura</p><h1>Verificando acesso</h1><p className="muted">Verificando sessão e vínculos autorizados...</p><LoadingBlock label="Carregando contexto seguro..." /></div></AuthStage>;
  ...
  if (state.session.memberships.length === 0) return <SecuritySurface state="membership-unavailable" retry={() => beginBootstrap(true)} retryAfter={null} />;

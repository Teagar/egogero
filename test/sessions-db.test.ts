...
test(
  ...
  async () => {
    ...
    try {
      ...
      const nextLogin = await issue(await createHandoff(), gatehouse.sessionToken);
      assert.ok(nextLogin);
      assert.equal(nextLogin.identity.role, 'portaria');
      ...
      await prisma.condominio.update({ where: { id: condominioB }, data: { deletedAt: new Date() } });
      assert.equal(await store.authenticate(nextLogin.sessionToken, 'deleted-condominium'), null);
      await prisma.condominio.update({ where: { id: condominioB }, data: { deletedAt: null } });
      ...
      await prisma.humanMembership.update({
        where: { id: gatehouseMembershipId },
      ...
      const revokedCount = await store.revokeAll({ accountId, requestCorrelationId: 'revoke-all' });
      ...
      await prisma.oidcValidatedHandoff.deleteMany({ where: { accountId } }).catch(() => undefined);
